...
### Font Family
**JetBrains Mono** is the open-source monospaced face used across every text role in the system. It carries weights 400 (regular), 500 (medium), and 700 (bold) and falls back through a long monospace stack — IBM Plex Mono → ui-monospace → SFMono-Regular → Menlo → Monaco → Consolas → Liberation Mono → Courier New.

The single-font decision is the brand. There is no display face, no body sans, no italic alternative, and no fallback to a proportional font anywhere — even the legal copyright row uses JetBrains Mono at 14px. This is the most aggressive typographic restraint of any site in the marketing-tools category: OpenCode's identity is "the marketing page is a man page."
...
### Note on Font Fallbacks
JetBrains Mono is the primary font. Use weights 400 / 500 / 700 and retain the following fallbacks:
- **IBM Plex Mono** — secondary fallback with slightly more open counters and compatible line-height behavior.
...
- Background `{colors.surface-card}` (`#f1eeee`), text `{colors.ink}` rendered in `{typography.body-md}` (already monospaced — JetBrains Mono), padding `12px 16px`, rounded `{rounded.sm}`.
...
7. When introducing a new component, ask whether it can be expressed with the existing ASCII-bracket + 4px-radius + JetBrains-Mono vocabulary before adding new tokens. The system's strength is that it almost never needs new ones.

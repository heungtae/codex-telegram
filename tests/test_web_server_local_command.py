...
class WebServerLocalCommandTests(unittest.TestCase):
    ...
    def test_chat_messages_passes_structured_items_to_turn_start(self):
        app = create_web_app()
        endpoint = next(
            route.endpoint
            for route in app.routes
        ...
        request = SimpleNamespace(cookies={COOKIE_NAME: self.session.token})
        state_user = user_manager.get(self.session.user_id)
        state_user.active_thread_id = "thread-structured"
        state_user.set_collaboration_mode("plan")
        state_user.set_collaboration_mode_mask(
            {"name": "plan", "mode": "plan", "model": "gpt-5.3-codex", "reasoning_effort": "medium"}
        )
        state.codex_client.call.return_value = {"turn": {"id": "turn-structured"}}
            ...
            endpoint(
                {
                    ...
                    "items": [
                        ...
                        {"type": "skill", "name": "clean-code", "path": "/tmp/skills/clean-code/SKILL.md"},
    ...
    def test_chat_messages_rejects_invalid_structured_items(self):
        app = create_web_app()
        endpoint = next(
            route.endpoint
            for route in app.routes
        ...
        request = SimpleNamespace(cookies={COOKIE_NAME: self.session.token})
        state_user = user_manager.get(self.session.user_id)
        state_user.active_thread_id = "thread-invalid"

        with self.assertRaises(HTTPException) as ctx:
            asyncio.run(endpoint({"items": [{"type": "mention", "name": "Github"}]}, request))

        self.assertEqual(400, ctx.exception.status_code)
        self.assertIn("mention item requires name and path", ctx.exception.detail)
        state.codex_client.call.assert_not_called()

    def test_chat_messages_rejects_structured_local_command_payload(self):
        app = create_web_app()
        endpoint = next(
            route.endpoint
            for route in app.routes
        ...
        self.assertEqual(400, ctx.exception.status_code)
        self.assertIn("structured input does not support local ! commands", ctx.exception.detail)
        state.codex_client.call.assert_not_called()
...
""".strip(),
    ...
    def test_thread_read_preserves_structured_user_inputs(self):
        app = create_web_app()
        endpoint = next(
            route.endpoint
            for route in app.routes
                        ...
                        "items": [
                            {
                                ...
                                "content": [
                                    ...
                                    {"type": "local_image", "path": "/tmp/local.png"},
                                    {"type": "image", "image_url": "https://example.com/remote.png"},
                                    {"type": "skill", "name": "clean-code", "path": "/tmp/skills/clean-code/SKILL.md"},
                                    {"type": "mention", "name": "Github", "path": "app://github"},
                                ],
                            }
        ...
        body = asyncio.run(endpoint(request, "thread-structured"))
            ...
            [
                {
                    ...
                    "text": "review this",
                    "thread_id": "thread-structured",
                    "images": ["https://example.com/remote.png"],
                    "local_images": ["/tmp/local.png"],
                    "skills": [{"name": "clean-code", "path": "/tmp/skills/clean-code/SKILL.md"}],
                    "mentions": [{"name": "Github", "path": "app://github"}],
                }
            ],
            body["messages"],
        )
    ...
    def test_skills_catalog_returns_structured_items(self):
        app = create_web_app()
        endpoint = next(
            route.endpoint
            for route in app.routes
            if getattr(route, "path", None) == "/api/skills/catalog"
        )
        request = SimpleNamespace(cookies={COOKIE_NAME: self.session.token})
        state.codex_client.call = AsyncMock(
            return_value={
                "data": [
                    {
                        "skills": [
                            {"displayName": "Clean Code", "pathToSkillsMd": "/tmp/skills/clean-code/SKILL.md"},
                        ]
                    }
                ]
            }
        ...
        self.assertEqual(
            {"items": [{"name": "Clean Code", "path": "/tmp/skills/clean-code/SKILL.md"}]},
            body,
        )

    def test_apps_catalog_returns_enabled_apps(self):
        app = create_web_app()
        endpoint = next(
            route.endpoint
            for route in app.routes
            if getattr(route, "path", None) == "/api/apps/catalog"
        )
        request = SimpleNamespace(cookies={COOKIE_NAME: self.session.token})
        state.codex_client.call = AsyncMock(
            return_value={
                "data": [
                    ...
                    {"id": "slack", "displayName": "Slack", "enabled": False},
                ]
            }
        )
        ...
        self.assertEqual({"items": [{"name": "GitHub", "path": "app://github"}]}, body)
